...
export class QueueService {
  ...
  private initializeQueues(): void {
    ...
    const notificationsQueue = new Queue<JobData>(QueueName.NOTIFICATIONS, {
      redis: {
    ...
    // Initialize emails queue
    const emailsQueue = new Queue<JobData>(QueueName.EMAILS, {
      redis: {
        host: process.env.REDIS_HOST || 'localhost',
        port: parseInt(process.env.REDIS_PORT || '6379'),
      },
    ...
    const reportsQueue = new Queue<JobData>(QueueName.REPORTS, {
      redis: {
        host: process.env.REDIS_HOST || 'localhost',
        port: parseInt(process.env.REDIS_PORT || '6379'),
      },
    ...
    // Handle failed jobs
    [notificationsQueue, emailsQueue, reportsQueue].forEach(queue => {
  ...
  private async processEventCreatedNotification(payload: { event: Event; employees: Employee[] }): Promise<void> {
    const { event, employees } = payload;
      ...
      employees
        ...
        .map(employee => 
          this.notificationService.createNotification(
            NotificationTemplates.eventCreated(event, employee.user!.id)
          ).catch(error => 
            this.logger.error(`Failed to send notification to employee ${employee.id}:`, error)
          )
        )
    );
    
    this.logger.info(`Successfully processed event created notifications for event ${event.id}`);
  }
  ...
  private async processEventInvitationNotification(payload: { event: Event; employee: Employee }): Promise<void> {
    ...
    if (employee.user) {
      await this.notificationService.createNotification(
        NotificationTemplates.eventInvitation(event, employee.user.id)
      );
  ...
  private async processEventReminderNotification(payload: { event: Event; participants: Employee[] }): Promise<void> {
    ...
    await Promise.all(
      participants
        ...
        .map(participant => 
          this.notificationService.createNotification(
            NotificationTemplates.eventReminder(event, participant.user!.id)
          ).catch(error => 
            this.logger.error(`Failed to send reminder notification to participant ${participant.id}:`, error)
          )
        )
    );
  ...
  private async processPostLikedNotification(payload: { sender: User, post: Post }): Promise<void> {
    await this.notificationService.createNotification(
      NotificationTemplates.postLiked(payload.sender, payload.post)
    );
  }

  private async processPostCommentedNotification(payload: { sender: User, post: Post, commentContent: string }): Promise<void> {
    await this.notificationService.createNotification(
      NotificationTemplates.postCommented(payload.sender, payload.post, payload.commentContent)
    );
  }

  private async processCommentRepliedNotification(payload: { sender: User, parentCommentAuthorId: number, commentContent: string, postId: number }): Promise<void> {
    await this.notificationService.createNotification(
      NotificationTemplates.commentReplied(payload.sender, payload.parentCommentAuthorId, payload.commentContent, payload.postId)
    );
  }

  private async processCommentLikedNotification(payload: { sender: User, commentAuthorId: number, commentContent: string, postId: number }): Promise<void> {
    await this.notificationService.createNotification(
  ...
  private async processPtoRequestCreatedNotification(payload: { ptoRequest: PtoRequest, recipientId: number }): Promise<void> {
    await this.notificationService.createNotification(
        NotificationTemplates.ptoRequestCreated(payload.ptoRequest, payload.recipientId)
    );
  }

  private async processPtoRequestStatusNotification(payload: { ptoRequest: PtoRequest, updatedBy: User }): Promise<void> {
    await this.notificationService.createNotification(
      NotificationTemplates.ptoRequestStatusUpdate(payload.ptoRequest, payload.updatedBy)
    );
  }
  ...
  private async processDepartmentTransferNotification(payload: { employee: Employee, fromDepartment: Department, toDepartment: Department, transferredBy: User }): Promise<void> {
    await this.notificationService.createNotification(
  ...
  private async processDepartmentCreatedNotification(payload: { department: Department, manager: Employee }): Promise<void> {
    await this.notificationService.createNotification(
      NotificationTemplates.departmentCreated(payload.department, payload.manager)
    );
  }
  ...
  public async getJobStatus(queueName: QueueName, jobId: string): Promise<{
    status: string;
    progress: number;
    attempts: number;
    timestamp: number;
