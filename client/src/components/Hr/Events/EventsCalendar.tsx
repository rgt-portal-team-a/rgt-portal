          ...
          {weekDays.map((day, index) => (
            <div
              ...
              className={`p-2 text-center text-sm border-r ${
                isToday(day) ? "bg-purple-100 font-bold" : ""
              }`}
            >
              <div>{getShortDay(index)}</div>
              <div
                className={`text-lg ${isToday(day) ? "text-purple-700" : ""}`}
              >
                {day.getDate()}
              </div>
            </div>
            ...
            return (
              <div
                ...
                className={`min-h-24 font-bold ${
                  !isDateInCurrentMonth ? "bg-gray-50" : "bg-white"
                }`}
              >
                <div
                  className={`text-left px-3 py-1 ${
                    !isDateInCurrentMonth ? "text-gray-400" : ""
                  } ${
                    isDateToday ? "bg-purple-100 text-purple-700 font-bold" : ""
                  }`}
                >
                  {day.getDate()}
                </div>
          ...
          {Array(daysInMonth)
            ...
            .map((_, i) => {
              ...
              const hasEvents = monthEvents.some(
                (event) => event.startTime.getDate() === day
              );
              ...
              return (
                <div
                  ...
                  className={`text-center py-1 ${
                    isDateToday
                      ? "bg-purple-100 rounded-full text-purple-700 font-bold"
                      : ""
                  }`}
                >
                  {day}
                  {hasEvents && (
                    <div className="mx-auto w-1 h-1 bg-purple-500 rounded-full mt-1"></div>
                  )}
                </div>
              );
            })}
        ...
        <div className="flex text-gray-400 border-b border-gray-300 mx-auto items-center justify-center overflow-hidden">
          {(["date", "week", "month", "year"] as const).map((viewOption) => (
            <button
              ...
              className={`px-4 py-1 ${
                view === viewOption
                  ? "bg-gray-100 border-b-4 border-gray-400"
                  : ""
              }`}
